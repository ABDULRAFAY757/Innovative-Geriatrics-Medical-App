...
const Header = ({ user, onLogout }) => {
  ...
  const roleColors = {
    patient: 'bg-gradient-to-br from-blue-500 to-blue-700',
    family: 'bg-gradient-to-br from-green-500 to-emerald-700',
    doctor: 'bg-gradient-to-br from-purple-500 to-indigo-700',
  };
  ...
  return (
    <header
      className={clsx(
        'bg-white/95 backdrop-blur-sm border-b border-gray-200/80 sticky top-0 z-40 shadow-sm',
        isRTL && 'font-arabic'
      )}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <div className={clsx('p-2 rounded-xl shadow-lg', roleColors[user?.role] || 'bg-blue-600')}>
              <Stethoscope className="w-6 h-6 text-white" />
            </div>
            <div className="hidden sm:block">
              <h1 className="text-lg font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">{t('app_name')}</h1>
              <p className="text-xs text-gray-500 capitalize">{t(user?.role || 'patient')}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-gradient-to-r from-red-500 to-red-600 rounded-full text-xs text-white flex items-center justify-center font-semibold shadow-lg animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              {isNotificationsOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-gray-200 z-20 overflow-hidden">
                    <div className="p-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                      <h3 className="font-bold text-gray-900">{t('notifications')}</h3>
                      <p className="text-xs text-gray-500">{unreadCount} {language === 'ar' ? 'غير مقروءة' : 'unread'}</p>
                    </div>
                      ...
                      {notifications.map((notif) => (
                        <div
                          ...
                          className={clsx(
                            ...
                            notif.unread && 'bg-blue-50/50 border-l-4 border-l-blue-500'
                          )}
                        >
                    ...
                    <div className="p-3 border-t border-gray-100 bg-gray-50">
                      <button className="w-full text-sm text-blue-600 hover:text-blue-700 font-semibold py-1 hover:bg-blue-50 rounded-lg transition-colors">
                        {t('view_all')}
                      </button>
                    </div>
                  </div>
